...
@section('content')
    ...
    {{ method_field('PUT') }}
    <h3>Cập nhật đơn hàng: {{ $order->code }}</h3>
    <form action="{{ route('admin.order.update', ['id' => $order->id]) }}" method="POST" novalidate>
        @csrf
                ...
                <table class="table table-bordered">
                    ...
                    @if (old('product_id'))
                        @if (old('product_id') && old('quantity'))
                            @php($count = count(old('product_id')))
                            @for ($i = 0; $i < $count; $i++)
                                <tr>
                                    <td>
                            ...
                            @for ($i = 0; $i < $count; $i++)
                                <tr>
                                    <td>
                                        <select name="product_id[]" id="{{ 'product_id'.$i }}"
                                            ...
                                            @if (count($product) > 0)
                                                @foreach ($product as $data)
                                                    <option value="{{ $data->id }}"
                                                        @if ($data->id == $order_product[$i]->product_id) {
                                                            {{'selected'}}
                                                        @else
                                                            {{''}}
                                                        @endif
                                    ...
                                    <td>
                                        <input type="number" class="form-control" name="quantity[]"
                                               id="{{ 'quantity'.$i }}"
                                               value="{{ $order_product[$i]->quantity }}"
                                        >
                                    </td>
                                    <td></td>
                                    <td id="{{ 'total_each'.$i }}"></td>
        ...
        <div class="form-group row">
            <label class="col-lg-4" for="total_money">Tổng giá trị đơn hàng:</label>
            <div class="col-lg-8">
        ...
        <div class="row">
            <div class="form-group col-lg-6">
                <div class="row">
                    <label class="col-lg-4" for="customer_id">Khách hàng(*):</label>
                    <div class="col-lg-8">
                                    ...
                                    <option value="{{ $data->id }}"
                                        ...
                                        @else
                                            @if ($order->customer_id) {
                                                {{ $order->customer_id == $data->id ? 'selected' : '' }}
                                            @endif
                                        @endif
                        ...
                        <select name="repository_id" id="repository_id" class="form-control">
                            ...
                            @if (count($repository) > 0)
                                @foreach ($repository as $data)
                                    <option value="{{ $data->id }}"
                                        @if (old('repository_id'))
                                            {{ old('repository_id') == $data->id ? 'selected' : '' }}
                                        @else
                                            @if ($order->repository_id) {
                                                {{ $order->repository_id == $data->id ? 'selected' : '' }}
                                            @endif
                                        @endif
                                    >{{ $data->name }}</option>
                                @endforeach
                                       ...
                                       value="1"
                                        ...
                                        @else
                                            @if ($order->payment_method)
                                                {{ $order->payment_method == 1 ? 'checked' : '' }}
                                            @endif
                                        @endif
                                >
                            ...
                            <div class="form-check col-lg-6">
                                <input class="form-check-input" type="radio" name="payment_method" id="payment_method2"
                                       value="2"
                                        ...
                                        @else
                                            @if ($order->payment_method)
                                                {{ $order->payment_method == 2 ? 'checked' : '' }}
                                            @endif
                                        @endif
                                >
